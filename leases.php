...
<div class="container mx-auto px-4 py-6 max-w-7xl">
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">DHCP Leases</h1>
        <p class="mt-2 text-sm text-gray-600">Manage active DHCP leases for switch and interface</p>
    </div>
    ...
    <div id="leasesTableContainer" class="bg-white shadow-md rounded-lg overflow-hidden hidden">
        ...
        <!-- Static Leases Table Container -->
        <div id="staticLeasesTableContainer" class="bg-white shadow-md rounded-lg overflow-x-auto hidden">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
            ...
            <tbody id="staticLeasesTableBody" class="bg-white divide-y divide-gray-200">
                <!-- Static leases content will be dynamically inserted here -->
            </tbody>
        </table>
    </div>
...
function toggleStaticLeases(button, subnetId) {
    const staticLeasesTableContainer = document.getElementById('staticLeasesTableContainer');

    if (staticLeasesTableContainer.classList.contains('hidden')) {
        // Show static leases
        staticLeasesTableContainer.classList.remove('hidden');
        button.textContent = 'Hide Static Leases';
        loadStaticLeases(subnetId);
    } else {
        // Hide static leases
        staticLeasesTableContainer.classList.add('hidden');
        button.textContent = 'View Static Leases';
    }
}

async function loadStaticLeases(subnetId) {
    const tableContainer = document.getElementById('staticLeasesTableContainer');
    const loadingIndicator = document.getElementById('loadingIndicatorStatic');
    const tableBody = document.getElementById('staticLeasesTableBody');
    const errorState = document.getElementById('errorStateStatic');
    ...
    try {
        ...
        if (!responseData.success || !responseData.data || !responseData.data.hosts) {
            throw new Error('Invalid data format received from server');
        }

        const hosts = responseData.data.hosts;
        ...
        if (hosts.length === 0) {
                ...
                <tr>
                    <td colspan="4" class="px-6 py-8 text-center">
                        <div class="flex flex-col items-center justify-center space-y-3">
                            ...
                            <span class="text-sm font-medium text-gray-500">No static leases found</span>
                        </div>
                    </td>
                </tr>
            `;
            ...
            tableBody.innerHTML = hosts.map(host => `
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">${host.duid || 'N/A'}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900">${host['option-data'] ? host['option-data'].length : 0} options</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-left text-sm font-medium">
                        <button onclick="editReservation('${host['ip-addresses'][0]}')" 
                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-2">
                            Edit
                        </button>
        ...
        console.error('Error loading static leases:', error);
        if (loadingIndicator) loadingIndicator.classList.add('hidden');
        if (errorState) {
            errorState.classList.remove('hidden');
            errorState.querySelector('#errorMessageStatic').textContent = `Error loading static leases: ${error.message}`;
        }
    }
}
...
async function viewStaticLeases(subnetId) {
    try {
        ...
        if (data.result === 0) {
            ...
            if (hosts.length === 0) {
                Swal.fire({
                    ...
                    text: 'No static leases found for this subnet',
                    icon: 'info'
                });
                return;
            }
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DUID</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">DHCP Options</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ${hosts.map(host => `
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ${host['option-data'] ? host['option-data'].length : 0} options
                                    </td>
        ...
        Swal.fire({
            title: 'Error',
            text: `Error loading reservations: ${error.message}`,
            icon: 'error'
        });
    }
}
...
async function addStaticLease() {
    ...
    // Validation checks
    if (!ipAddressSuffix || !duid || options.length === 0) {
...
function validateOptionValue(input) {
    ...
    if (!select.value) {
        ...
        showError(input, errorElement, 'Please select a DHCP option first');
        return false;
    }
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-3">
                    <div class="flex space-x-2">
                        ...
                        <button onclick="toggleStaticLeases('${subnets[0].bvi_interface_id}')" 
